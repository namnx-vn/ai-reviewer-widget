...
export const REAL_WORLD_PR_CATALOG: readonly RealWorldCatalogEntry[] = [
  ...
  pr("vercel/next.js", 95574, "Add experimental WebSocket route handlers", "security", "positive-candidate"),
  pr("vercel/next.js", 96449, "Harden and classify Request Insights activity", "security", "positive-candidate"),
  pr("vercel/next.js", 96902, "Establish Request Insights classification and isolation", "security", "positive-candidate"),
  pr("vercel/next.js", 98265, "fix(next/image): follow redirects from a relative image src (#98237)", "security", "positive-candidate"),
  pr("vercel/next.js", 96608, "fix(app-render): add nonce to loading/template/error segment script tags", "security", "positive-candidate"),
  pr("vercel/next.js", 97043, "Thread CSP nonce to the React render in Pages Router", "security", "positive-candidate"),
  pr("vercel/next.js", 98152, "Add CSP nonce to script tags of loading and template files", "security", "positive-candidate"),
  pr("vercel/next.js", 98197, "Add Next Maintainer auto-close workflow", "security", "manual-review"),
  pr("vercel/next.js", 97027, "Add bounded WebSocket transport primitives", "security", "positive-candidate"),
  pr("TanStack/query", 11381, "docs(preact-query): fix XSS-unsafe example and missing staleTime in SSR guide", "security", "positive-candidate"),
  pr("vercel/next.js", 95123, "inherit allowedDevOrigins into serverActions.allowedOrigins", "security", "manual-review"),
  pr("vercel/next.js", 95238, "fix(cache-components): decompress postponed resume body before parsing", "security", "positive-candidate"),
  pr("vercel/next.js", 97252, "Add a script for adopting fork pull requests", "security", "manual-review"),
  pr("vercel/next.js", 96552, "Preserve the original host on forwarded Server Action requests", "security", "manual-review"),
  pr("vercel/next.js", 96977, "Preserve the original host on action redirect streams", "security", "manual-review"),
  pr("vercel/next.js", 98105, "Respond with 400 when a Server Action body cannot be decoded", "security", "manual-review"),
  pr("vercel/next.js", 95182, "server actions: enforce bodySizeLimit in edge runtime for multipart and non-multipart bodies", "security", "positive-candidate"),
  pr("vercel/next.js", 96580, "Keep loaded env when a forced .env reload reads nothing", "security", "manual-review"),

  // React / hooks / reactive lifecycle — 15
  pr("TanStack/query", 11385, "fix(solid-query): prevent lost updates when mounting cached queries", "react-hooks", "positive-candidate"),
  pr("TanStack/query", 10006, "fix(devtools): isolate instance-specific settings with context", "react-hooks", "positive-candidate"),
  pr("TanStack/query", 10079, "fix(react-query): retry useQuery().promise on error boundary reset", "react-hooks", "positive-candidate"),
  pr("TanStack/query", 11403, "test(vue-query/useMutation): cover optimistic rollback and concurrent calls", "react-hooks", "manual-review"),
  pr("TanStack/query", 11402, "test(vue-query/useInfiniteQuery): cover reactive options and page transitions", "react-hooks", "manual-review"),
  pr("TanStack/query", 11270, "fix(examples/vue/nuxt3): guard hydrate() against a null dehydrated state", "react-hooks", "positive-candidate"),
  pr("TanStack/query", 11260, "fix(query-core): accept partial dehydrated state", "react-hooks", "positive-candidate"),
  pr("TanStack/query", 11326, "feat(solid-query): built-in single-flight consumer via FLIGHT_DATA_SOURCE", "react-hooks", "manual-review"),
  pr("TanStack/query", 11308, "feat(solid-query): rewrite the adapter onto Solid 2.0 native async model", "react-hooks", "manual-review"),
  pr("TanStack/query", 11395, "feat(query-core): allow overriding dehydration timestamp", "react-hooks", "manual-review"),
  pr("vercel/next.js", 96252, "Fix race when navigating Back before hydration", "react-hooks", "positive-candidate"),
  pr("vercel/next.js", 83200, "fix: web manifest should always be emitted in head", "react-hooks", "positive-candidate"),
  pr("vercel/next.js", 91586, "perf: skip context.with() and span overhead for noop tracer", "react-hooks", "manual-review"),
  pr("vercel/next.js", 93154, "fix(router): preserve multi-value search params in segment cache key", "react-hooks", "positive-candidate"),
  pr("vercel/next.js", 96735, "Upgrade React from 7dfc7ccd-20260803 to 11eddecd-20260805", "react-hooks", "manual-review"),
  ...
  pr("TanStack/query", 11406, "docs(angular-query-experimental): add JSDoc across the package", "clean", "negative-control"),
  pr("TanStack/query", 11407, "test(query-core): expand notifyManager unit tests", "clean", "negative-control"),
  pr("TanStack/query", 11393, "test(query-core): expand TimeoutManager unit tests", "clean", "negative-control"),
  pr("vercel/next.js", 98291, "docs: add a section on measuring memory at runtime", "clean", "negative-control"),
  pr("vercel/next.js", 96751, "docs: present each Skill as steps in the AI agents guide", "clean", "negative-control"),
  pr("vercel/next.js", 96723, "docs: update redirected links to current targets", "clean", "negative-control"),
  pr("vercel/next.js", 75178, "Update README.md", "clean", "negative-control"),
  pr("vercel/next.js", 96871, "Lint devlow-bench with the root eslint config", "clean", "negative-control"),
  pr("vercel/next.js", 96772, "Consolidate Promise.withResolvers polyfills", "clean", "negative-control"),
  pr("vercel/next.js", 97284, "introduce an options struct for constructing backend storage", "clean", "negative-control"),
  pr("TanStack/query", 11366, "docs(react-query): generate reference docs with TypeDoc", "clean", "negative-control"),
  pr("TanStack/query", 11380, "docs(preact-query): add Server Rendering & Hydration guide", "clean", "negative-control"),
  pr("TanStack/query", 11369, "docs(solid-query): add JSDoc and generate reference docs with TypeDoc", "clean", "negative-control"),
  pr("TanStack/query", 11368, "chore(scripts/generate-docs): add solid-query entry point", "clean", "negative-control"),
  pr("TanStack/query", 11378, "docs(vue-query): add JSDoc and generate reference docs with TypeDoc", "clean", "negative-control"),
];
