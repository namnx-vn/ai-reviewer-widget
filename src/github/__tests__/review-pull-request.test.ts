...
describe("GitHub pull request review adapter", () => {
  it("converts only reviewable files and loads base content only for modified files", async () => {
    ...
    expect(converted.files).toEqual([
      {
        path: "src/changed.ts",
        content: "head-sha:src/changed.ts",
        patch: "@@ -1 +1 @@\n-old\n+new",
        changedLines: [1],
      },
      {
        path: "src/new.tsx",
        content: "head-sha:src/new.tsx",
        patch: "@@ -0,0 +1 @@\n+export const New = () => null;",
        changedLines: [1],
      },
    ]);
    expect(converted.baseFiles).toEqual([{
      path: "src/changed.ts",
      content: "base-sha:src/changed.ts",
    }]);
  ...
  it("fetches, reviews, filters inline findings, and publishes both GitHub outputs", async () => {
    ...
    const aiReviewer: AIReviewerPort = {
      name: "test-ai",
    ...
    expect(onPullRequestLoaded).toHaveBeenCalledWith(expect.objectContaining({
      number: 17,
      title: "Tighten review",
    }));
    expect(reviewPullRequest).toHaveBeenCalledWith(expect.objectContaining({
      title: "Tighten review",
      description: "PR body",
      securityQualityGate: expect.objectContaining({
    ...
    expect(client.createPullRequestReview).toHaveBeenCalledWith(
      "acme", "widget", 17, "head-sha", result.findings.slice(0, 1),
    );
