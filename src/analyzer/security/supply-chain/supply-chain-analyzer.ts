...
type SupplyChainKind =
  | "git-dependency"
  | "http-dependency"
  | "install-script"
  | "dynamic-require"
  | "lockfile-missing"
  | "unpinned-critical-source";
...
const METADATA: Readonly<Record<SupplyChainKind, SecurityRuleMeta>> = {
  ...
  "http-dependency": meta(
    "security.supply-chain.http-dependency",
    "HTTP dependency source",
    "high",
    "CWE-494",
  ),
  "install-script": meta(
...
export function analyzeSupplyChain(
  ...
  const matches: Match[] = [
    ...
    ...analyzeLockfiles(
      ...
      repository.lockfiles.map((lockfile) => lockfile.path),
    ),
    ...analyzeSourceFiles(repository),
  ];

  return matches.sort(compareMatches).map(createFinding);
}

function analyzeManifests(
  manifests: readonly SupplyChainManifest[],
  criticalSources: readonly string[],
): readonly Match[] {
  ...
  return manifests.flatMap((manifest) => {
    ...
    for (const [name, version] of dependenciesOf(manifest)) {
      ...
      if (isHttpDependency(version))
        matches.push({
          kind: "http-dependency",
          path: manifest.path,
          detail: name,
        });
      if (critical.has(name) && !isExactVersion(version))
        matches.push({
          kind: "unpinned-critical-source",
          path: manifest.path,
          detail: name,
        });
    }
...
function analyzeLockfiles(
  ...
  return manifests
    ...
    .map((manifest) => ({
      kind: "lockfile-missing" as const,
      path: manifest.path,
  ...
  const location = {
    ...
    range:
      match.range === undefined
        ? undefined
        : { start: match.range[0], end: match.range[1] },
  };
  return {
    ...
    evidence: [
      {
        message: `Detected ${messageFor(match.kind, match.detail).toLowerCase()} without retaining external values.`,
        location,
      },
    ],
...
function meta(
  ...
  return {
    id,
    title,
    description: `Detects ${title.toLowerCase()} using offline repository metadata.`,
    category: "supply-chain",
...
function isGitDependency(version: string): boolean {
  return /^(?:git\+|github:|git:|git@)/i.test(version);
}
function isHttpDependency(version: string): boolean {
  return /^http:\/\//i.test(version);
}
...
function hasLockfileForManifest(
  manifestPath: string,
  lockfilePaths: readonly string[],
): boolean {
...
function compareMatches(left: Match, right: Match): number {
  return (
    ...
    (left.range?.[0] ?? -1) - (right.range?.[0] ?? -1) ||
    METADATA[left.kind].id.localeCompare(METADATA[right.kind].id) ||
    left.detail.localeCompare(right.detail)
  );
}
function messageFor(kind: SupplyChainKind, detail: string): string {
  const subject = detail ? ` for \`${detail}\`` : "";
  return {
    "git-dependency": `Git-based dependency source detected${subject}.`,
    "http-dependency": `Unencrypted HTTP dependency source detected${subject}.`,
    "install-script": `Package lifecycle script \`${detail}\` can execute during installation.`,
    "dynamic-require": "Module is loaded from a non-literal expression.",
    "lockfile-missing": "No lockfile was provided for this package manifest.",
    "unpinned-critical-source": `Critical dependency is not pinned to an exact version${subject}.`,
  }[kind];
}
function suggestionFor(kind: SupplyChainKind): string {
  return {
    "git-dependency":
      "Use a registry package with a reviewed, immutable version where possible.",
    "http-dependency": "Use HTTPS or a trusted package registry source.",
    "install-script":
      "Review lifecycle scripts and minimize installation-time code execution.",
    "dynamic-require":
      "Resolve module names from an allowlist of literal module specifiers.",
    "lockfile-missing":
      "Commit the workspace lockfile used for reproducible installs.",
    "unpinned-critical-source":
      "Pin this critical dependency to an exact reviewed version.",
  }[kind];
}
