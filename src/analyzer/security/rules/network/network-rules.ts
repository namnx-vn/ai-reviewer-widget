...
const METAS: Readonly<Record<NetworkKind, SecurityRuleMeta>> = {
  ...
  "permissive-cors": meta(
    "security.network.permissive-cors",
    "Permissive CORS origin",
    "medium",
    "CWE-942",
  ),
  "credentials-wildcard-origin": meta(
    "security.network.credentials-wildcard-origin",
    "CORS credentials with wildcard origin",
    "high",
    "CWE-942",
  ),
  "untrusted-proxy": meta(
    "security.network.untrusted-proxy",
    "Overly broad trusted proxy",
    "medium",
    "CWE-441",
  ),
};

export const networkTransportRules: readonly SecurityRule[] = (
  Object.keys(METAS) as NetworkKind[]
).map((kind) => ({
  meta: METAS[kind],
  check(context) {
...
function collectMatches(ast: TSESTree.Program): readonly NetworkMatch[] {
  ...
  visit(ast, (node) => {
    ...
    if (isTlsCall(name, path, state))
      addTlsOptions(matches, optionsArgument(node));
    if (node.type === "CallExpression" && state.corsCalls.has(name ?? ""))
      addCorsOptions(matches, argumentAt(node, 0));
    if (node.type === "CallExpression" && isProxySetCall(name, path, state))
      addProxySetting(matches, node);
  });
...
function buildModelState(ast: TSESTree.Program): ModelState {
  ...
  visit(ast, (node) => {
      ...
      for (const specifier of node.specifiers) {
        ...
        if (HTTP_MODULES.has(module)) {
          if (
            ...
            httpNamespaces.add(local);
          else if (TLS_METHODS.has(importedName(specifier)))
            httpCalls.add(local);
        }
    ...
    if (
      ...
      expressCalls.has(calleeName(node.init.callee) ?? "")
    )
      expressApps.add(name);
    if (
      node.init.type === "MemberExpression" &&
      node.init.object.type === "Identifier" &&
      expressApps.has(node.init.object.name) &&
      propertyName(node.init.property) === "set"
    )
      proxySetCalls.add(name);
  });
  return {
    httpNamespaces,
    httpCalls,
...
function addInsecureUrl(
  matches: NetworkMatch[],
  value: TSESTree.Node | undefined,
  fallback: TSESTree.Node,
): void {
  const url = stringValue(value);
  if (url !== undefined && /^http:\/\//i.test(url) && !isLocalUrl(url))
    matches.push({
      kind: "insecure-http",
      node: value ?? fallback,
      evidence: "Explicit HTTP endpoint uses an unencrypted transport.",
    });
}

function addWebSocketUrl(
  matches: NetworkMatch[],
  value: TSESTree.Node | undefined,
  fallback: TSESTree.Node,
): void {
  const url = stringValue(value);
  if (url !== undefined && /^ws:\/\//i.test(url) && !isLocalUrl(url))
    matches.push({
      kind: "insecure-websocket",
      node: value ?? fallback,
      evidence: "Explicit WebSocket endpoint uses an unencrypted transport.",
    });
}

function addTlsOptions(
  matches: NetworkMatch[],
  value: TSESTree.Node | undefined,
): void {
  ...
  if (
    rejectUnauthorized?.type === "Literal" &&
    rejectUnauthorized.value === false
  )
    matches.push({
      kind: "tls-verification-disabled",
      node: rejectUnauthorized,
      evidence: "TLS rejectUnauthorized is explicitly false.",
    });
  const minVersion = stringValue(objectValue(value, "minVersion"));
  if (minVersion !== undefined && WEAK_TLS.has(minVersion.toLowerCase()))
    matches.push({
      kind: "weak-tls",
      node: objectValue(value, "minVersion") ?? value,
      evidence: `TLS minimum version is ${minVersion}.`,
    });
  const secureProtocol = stringValue(objectValue(value, "secureProtocol"));
  if (
    secureProtocol !== undefined &&
    /(?:tlsv1|sslv[23])/i.test(secureProtocol)
  )
    matches.push({
      kind: "weak-tls",
      node: objectValue(value, "secureProtocol") ?? value,
      evidence: `TLS secure protocol is ${secureProtocol}.`,
    });
}
...
function addCorsOptions(
  ...
  matches.push({
    ...
    node: objectValue(value, "origin") ?? value,
    evidence: "CORS origin is explicitly configured as '*'.",
  });
  const credentials = objectValue(value, "credentials");
  if (credentials?.type === "Literal" && credentials.value === true)
    matches.push({
      kind: "credentials-wildcard-origin",
      node: value,
      evidence: "CORS enables credentials while allowing every origin.",
    });
}

function addProxySetting(
  matches: NetworkMatch[],
  ...
  if (
    value?.type === "Literal" &&
    (value.value === true || typeof value.value === "number")
  )
    matches.push({
      kind: "untrusted-proxy",
      node: value,
      evidence:
        "Express trust proxy is enabled broadly by a boolean or hop count.",
    });
}
...
function isHttpCall(
  ...
  return (
    path !== undefined &&
    path.length === 2 &&
    state.httpNamespaces.has(path[0] ?? "") &&
    TLS_METHODS.has(path[1] ?? "")
  );
}

function isWebSocketCall(
  name: string | undefined,
  path: readonly string[] | undefined,
): boolean {
  return name === "WebSocket" || path?.join(".") === "globalThis.WebSocket";
}
function isTlsCall(
  name: string | undefined,
  path: readonly string[] | undefined,
  ...
  return (
    path !== undefined &&
    path.length === 2 &&
    state.tlsNamespaces.has(path[0] ?? "") &&
    TLS_METHODS.has(path[1] ?? "")
  );
}
function isProxySetCall(
  name: string | undefined,
  path: readonly string[] | undefined,
  state: ModelState,
): boolean {
  return (
    state.proxySetCalls.has(name ?? "") ||
    (path?.length === 2 &&
      state.expressApps.has(path[0] ?? "") &&
      path[1] === "set")
  );
}
...
function memberPath(node: TSESTree.Node): readonly string[] | undefined {
  ...
  return base === undefined || property === undefined
    ? undefined
    : [...base, property];
}
function importedName(
  specifier: TSESTree.ImportDeclaration["specifiers"][number],
): string {
...
function isLocalUrl(url: string): boolean {
  return /^\w+:\/\/(?:localhost|127(?:\.\d{1,3}){3}|\[::1\])(?::|\/|$)/i.test(
    url,
  );
}
function unique(matches: readonly NetworkMatch[]): readonly NetworkMatch[] {
...
function meta(
  ...
  return {
    id,
    title,
    description: `${title} is configured explicitly.`,
    category: "network",
...
function createFinding(
  ...
  return {
    ...
    location,
    evidence: [
      { message: match.evidence, location, sinkKind: "network-request" },
    ],
    standards: metaForMatch.standards,
    sinkKind: "network-request",
    suggestion:
      "Use an explicit secure transport configuration appropriate for the deployment environment.",
  };
}
function locationOf(
  node: TSESTree.Node,
