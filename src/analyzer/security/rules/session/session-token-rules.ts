...
function hasSameSite(properties: ReadonlyMap<string, boolean | TSESTree.Node>): boolean { const value = properties.get("sameSite"); return isNode(value) && value.type === "Literal" && typeof value.value === "string" && /^(?:lax|strict|none)$/i.test(value.value); }
function hasNoneAlgorithm(properties: ReadonlyMap<string, boolean | TSESTree.Node>): boolean { const algorithms = properties.get("algorithms") ?? properties.get("algorithm"); if (!isNode(algorithms)) return false; if (algorithms.type === "Literal") return algorithms.value === "none"; return algorithms.type === "ArrayExpression" && algorithms.elements.some((element) => element?.type === "Literal" && element.value === "none"); }
