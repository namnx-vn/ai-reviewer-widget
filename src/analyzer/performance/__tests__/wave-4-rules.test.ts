...
function findings(
  ...
  databaseAdapters?: readonly {
    readonly callPaths: readonly string[];
    readonly collectionMethods?: readonly string[];
  }[],
): readonly string[] {
...
describe("wave 4 performance rules", () => {
  it("checks image delivery attributes", () => {
    expect(findings('const View = () => <img src="x" />;', assetPerformanceRules)).toEqual([
      "performance.image",
  ...
  it("finds repeated CPU work", () => {
    expect(findings(
      'for (const x of xs) { for (const y of ys) {} items.sort(); JSON.stringify(x); }',
      cpuPerformanceRules,
    )).toEqual([
  ...
  it("uses only configured database adapters", () => {
    expect(findings(
      'for (const id of ids) { db.user.find(id); } db.user.list();',
      databasePerformanceRules,
      [{
        callPaths: ["db.user.find", "db.user.list"],
        collectionMethods: ["db.user.list"],
      }],
    )).toEqual([
    ...
    expect(findings(
      'for (const id of ids) { db.user.find(id); }',
      databasePerformanceRules,
    )).toEqual([]);
  });
});
