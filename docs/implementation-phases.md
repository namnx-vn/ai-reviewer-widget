...
| 4.3 — Distributable CLI | Package the shared local review workflow as a stable executable. | Completed | Node CLI bundle, explicit bin/package surface, deterministic help/version and file ordering, text/JSON output contracts, and packed-artifact smoke tests. | `scripts/ai-reviewer.ts`, `src/cli/`, `vite.cli.config.ts`, `package-tests/`, `package.json` | The self-contained packed analyzer bundle is about 11.2 MB unpacked; registry publication remains out of scope. |
| 4.4 — CI Adapters & Portable Review Output | Run the shared reviewer predictably in CI and publish portable results. | Completed | Versioned CI result/exit contracts, canonical JSON, isolated SARIF conversion, staged GitHub analysis/publication, sanitized failure classification, summaries, and workflow artifacts. | `src/ci/`, `src/github/review-pull-request.ts`, `scripts/review-pr.ts`, `.github/workflows/ai-review.yml` | SARIF is retained as an artifact rather than uploaded to GitHub code scanning; global branch coverage remains below the 80% ideal. |
